...
impl Player {
    ...
    pub async fn send_packet(&mut self, packet: ServerPacket) -> Result<(), Box<dyn std::error::Error>> {
...
pub async fn send_player_list_update(players: &PlayerRegistry) -> Result<(), Box<dyn std::error::Error>> {
    let player_list = get_player_list(players).await;
    ...
    for (addr, username, _) in player_list {
        ...
        if let Some(player_arc) = players_lock.get(&addr) {
            ...
            player.send_packet(ServerPacket::PlayerListItem {
